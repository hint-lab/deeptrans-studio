...
export async function startBatchQAAction(itemIds: string[], opts: { targetLanguage?: string; domain?: string; tenantId?: string }) {
  ...
  const items = rows.filter((r) => (r.sourceText || '').trim().length > 0 || ((r as any).targetText || '').trim().length > 0) as Array<{ id: string; sourceText: string | null; targetText: string | null }>
