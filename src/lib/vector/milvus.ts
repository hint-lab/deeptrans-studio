...
const getRowCountFromStats = (stats: unknown): number => {
    ...
    const value =
        typeof entry?.value === 'string' ? Number(entry.value) : Number(entry?.value ?? 0);
...
export async function ensureCollection(params: {
    ...
    if (!exists.value) {
        ...
        await c.createCollection({
            ...
            fields: [
                ...
                { name: 'text', data_type: DataType.VarChar, max_length: 8192 },
                {
                    ...
                    type_params: { dim: String(params.dim) } as any,
                },
        ...
        try {
            const info: any = await (c as any).describeCollection?.({
                collection_name: params.collection,
            });
            const fields: any[] = info?.schema?.fields || info?.data?.schema?.fields || [];
            const hasId = fields.some(
                (f: any) => f.name === 'id' && f.data_type === DataType.VarChar
            );
            const hasText = fields.some(
                (f: any) => f.name === 'text' && f.data_type === DataType.VarChar
            );
            const vec = fields.find((f: any) => f.name === 'vector');
            const hasVec = !!vec && vec.data_type === DataType.FloatVector;
            const configuredDim = Number(
                vec?.type_params?.dim || vec?.params?.dim || vec?.dim || 0
            );
            const hasMeta = fields.some(
                (f: any) => f.name === 'meta' && f.data_type === DataType.VarChar
            );
            if (
                !hasId ||
                !hasText ||
                !hasVec ||
                !hasMeta ||
                (configuredDim && configuredDim !== params.dim)
            ) {
                try {
...
export async function upsertVectors(params: {
    ...
    // 先查看集合的实际字段
    try {
        const desc: any = await (c as any).describeCollection?.({
            collection_name: params.collection,
        });
    ...
    const entities = params.points.map(p => {
        ...
        // 确保向量是有效的数字数组
        const vector = Array.isArray(p.vector)
            ? p.vector.filter(v => typeof v === 'number' && !isNaN(v))
            : [];
    ...
    try {
        ...
        while (retries > 0) {
            ...
            try {
                const stats = await c.getCollectionStatistics({
                    collection_name: params.collection,
                });
...
function fuseResults(
    ...
    if (!fusionStrategy) {
        ...
        return combined.sort((a, b) => b.score - a.score).slice(0, finalTopK);
    }
