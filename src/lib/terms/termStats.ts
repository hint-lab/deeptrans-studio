...
import { extractCjkTerms, extractLatinTokens } from './tokenize';
...
export function buildStatCandidates(
    ...
    chunks.forEach((c, idx) => {
        ...
        // Latin word n-grams remain useful and readable. Chinese multi-word
        // phrases are already generated by extractCjkTerms without spaces.
        for (let i = 0; i < latin.length; i++) {
            const key = String(latin[i] || '').trim();
            if (!key) continue;
            unigramCounts.set(key, (unigramCounts.get(key) ?? 0) + 1);
            totalUnigrams += 1;
            const left = latin[i - 1];
            const right = latin[i + 1];
            if (left) {
            ...
            if (i + 1 < latin.length) {
                const bg = `${key} ${String(latin[i + 1])}`;
                bigramCounts.set(bg, (bigramCounts.get(bg) ?? 0) + 1);
            }
            if (i + 2 < latin.length) {
                const tg = `${key} ${String(latin[i + 1])} ${String(latin[i + 2])}`;
