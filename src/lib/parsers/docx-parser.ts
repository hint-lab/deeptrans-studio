...
export async function extractDocxFromUrl(url: string): Promise<{ text: string; html?: string; contentType?: string; structured?: any }> {
...
async function parseDocxStructure(buffer: Buffer): Promise<any> {
  ...
  const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '@_', removeNSPrefix: true, allowBooleanAttributes: true, trimValues: false });
  ...
  const numIdToDef: Record<string, { abstractNumId: string; lvls: Record<string, { format?: string; text?: string }> }> = {};
  try {
    const abstractNums = numbering?.numbering?.abstractNum;
    const nums = numbering?.numbering?.num;
    const absArr = Array.isArray(abstractNums) ? abstractNums : abstractNums ? [abstractNums] : [];
  ...
  function runsFromParagraph(p: any): Array<{ text: string; br?: boolean; bold?: boolean; italic?: boolean; underline?: boolean; color?: string; sizePt?: number; font?: string }> {
    try {
      const rArr = Array.isArray(p?.r) ? p.r : p?.r ? [p.r] : [];
      const out: Array<{ text: string; br?: boolean; bold?: boolean; italic?: boolean; underline?: boolean; color?: string; sizePt?: number; font?: string }> = [];
      for (const r of rArr) {
        // 换行 run
        if (r?.br) { out.push({ text: '', br: true }); continue; }
        let t = '';
        const rawt = r?.t;
        ...
        const uVal = rPr?.u ? (typeof rPr.u === 'object' ? rPr.u['@_val'] : rPr.u) : undefined;
        const underline = !!(uVal && uVal !== 'none' && uVal !== '0');
        const colorVal = rPr?.color?.['@_val'];
        const color = (typeof colorVal === 'string' && colorVal.toLowerCase() !== 'auto') ? `#${colorVal.replace(/^#/, '')}` : undefined;
  ...
  const lists: Array<{ level: number; ordered: boolean; text: string; numId?: string; ilvl?: number }> = [];
  const links: Array<{ text: string; href: string }> = [];
  const images: Array<{ rid: string; target: string; contentType?: string }> = [];
  const tables: Array<{ rows: Array<Array<{ text: string; colspan?: number; rowspan?: number }>> }> = [];
  const footnoteMap: Record<string, string> = {};
  const paragraphs: Array<{ level?: number | null; styleName?: string; text: string; runs: Array<{ text: string; br?: boolean; bold?: boolean; italic?: boolean; underline?: boolean; color?: string; sizePt?: number; font?: string }>; placeholderSpans?: PlaceholderSpan[] }> = [];
  ...
  try {
    ...
    for (const n of arr) {
      ...
      const txt = paras.map((pp: any) => textFromRuns(pp)).filter(Boolean).join('\n');
  ...
  try {
    ...
    const walk = (node: any) => {
      ...
      if (node.drawing && node.drawing?.inline?.graphic?.graphicData?.pic?.blipFill?.blip?.['@_embed']) {
        mediaIds.push(String(node.drawing.inline.graphic.graphicData.pic.blipFill.blip['@_embed']));
      }
    ...
    for (const tbl of tbllist) {
      ...
      for (const tr of trArr) {
        ...
        for (const tc of tcArr) {
          ...
          const text = pArr.map((pp: any) => textFromRuns(pp)).filter(Boolean).join('\n');
  ...
  try {
    ...
    const escapeHtml = (s: string) => String(s)
      ...
      .replace(/"/g, '&quot;')
      .replace(/'/g, '&#39;');
    const runsToHtml = (runs: Array<{ text: string; br?: boolean; bold?: boolean; italic?: boolean; underline?: boolean; color?: string; sizePt?: number; font?: string }>) => {
      const parts: string[] = [];
      for (const r of runs) {
        if (r.br) { parts.push('<br/>'); continue; }
        const style: string[] = [];
  ...
  const footnoteArr = Object.keys(footnoteMap).map(k => ({ id: Number(k), text: footnoteMap[k] }));
  ...
  return { outline, lists, tables, links, footnotes: footnoteArr, images, paragraphs, text: previewText, html: previewHtml };
}
