...
export function wordDiff(a: string, b: string): {
  ...
  if (n * m > 300000) {
    ...
    let endA = n - 1;
    let endB = m - 1;
    while (endA >= start && endB >= start && aTokens[endA] === bTokens[endB]) { endA--; endB--; }
    const baseline: Array<{ text: string; type: 'equal' | 'del' }> = [];
    const embedded: Array<{ text: string; type: 'equal' | 'ins' }> = [];
    for (let i = 0; i < start; i++) { baseline.push({ text: String(aTokens[i] ?? ''), type: 'equal' }); embedded.push({ text: String(bTokens[i] ?? ''), type: 'equal' }); }
    for (let i = start; i <= endA; i++) baseline.push({ text: String(aTokens[i] ?? ''), type: 'del' });
    for (let j = start; j <= endB; j++) embedded.push({ text: String(bTokens[j] ?? ''), type: 'ins' });
    for (let i = endA + 1, j = endB + 1; i < n && j < m; i++, j++) { baseline.push({ text: String(aTokens[i] ?? ''), type: 'equal' }); embedded.push({ text: String(bTokens[j] ?? ''), type: 'equal' }); }
  ...
  const set = (i: number, j: number, v: number) => { if (i >= 0 && j >= 0 && i <= n && j <= m) dp[i * cols + j] = v; };
  for (let i = n - 1; i >= 0; i--) {
    for (let j = m - 1; j >= 0; j--) {
  ...
  while (i < n) { baseline.push({ text: String(aTokens[i] ?? ''), type: 'del' }); i++; }
  while (j < m) { embedded.push({ text: String(bTokens[j] ?? ''), type: 'ins' }); j++; }
  return { baseline, embedded };
}
