...
export type TranslatePhase = 'mono-term-extract' | 'dict-lookup' | 'term-embed-trans' | 'translate' | 'all'
...
export const workFlowStepSlice = createSlice({
  ...
  reducers: {
    ...
    setPreOutputs(
      ...
      if (partial.terms !== undefined) {
        ...
        for (const t of list as any[]) {
          const key = typeof t === 'string' ? t : String((t as any)?.term ?? (t as any)?.source ?? '')
          if (!key) continue
    ...
    setQAOutputs(state, action: PayloadAction<{ biTerm?: any; syntax?: any; discourse?: any } | undefined>) {
      const partial = action.payload
      if (!partial) {
    ...
    setPosteditOutputs(state, action: PayloadAction<{ memos?: any[]; discourse?: any; result?: any } | undefined>) {
      const partial = action.payload
      if (!partial) {
