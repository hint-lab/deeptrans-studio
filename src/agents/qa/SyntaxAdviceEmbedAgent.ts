...
export class SyntaxAdviceEmbedAgent extends BaseAgent<{ source: string; target: string; issues: Array<{ type?: string; span?: string; advice?: string }>; prompt?: string; locale?: string }, string> {
  ...
  async execute(input: { source: string; target: string; issues: Array<{ type?: string; span?: string; advice?: string }>; prompt?: string; locale?: string }, ctx?: AgentRunContext): Promise<string> {
    ...
    const formatted = list.map((a, i) => `#${i + 1} [${a.type || 'ISSUE'}] ${a.span ? `片段: ${a.span} | ` : ''}${a.advice || ''}`).join('\n');

    const sourceText = i18n.getUserPrompt('source_text', { text: input.source || '' });
    const currentTranslation = i18n.getUserPrompt('current_translation', { text: input.target || '' });
    const suggestionsToApply = i18n.getUserPrompt('suggestions_to_apply', { suggestions: formatted || '(空)' });
