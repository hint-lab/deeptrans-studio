...
export class SyntaxMarkerExtractAgent extends BaseAgent<{ source: string; target: string; prompt?: string; locale?: string }, any> {
  ...
  async execute(input: { source: string; target: string; prompt?: string; locale?: string }, ctx?: AgentRunContext): Promise<any> {
    ...
    const establishCorrespondence = i18n.getAgentPrompt('syntax_marker_extract', 'establish_correspondence');
