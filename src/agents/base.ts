...
export abstract class BaseAgent<TInput, TOutput> {
  ...
  protected async buildPrompt(outputFormat: 'json' | 'text' = 'text', constraints?: string[]): Promise<string> {
  ...
  protected async buildGlossary(dict?: Array<{ term: string; translation: string }>, maxItems: number = 200): Promise<string> {
  ...
  protected messages(systemContent: string, userContent: string): Array<{ role: 'system' | 'user'; content: string }> {
    return [
