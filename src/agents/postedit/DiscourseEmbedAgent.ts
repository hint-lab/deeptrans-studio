...
export class DiscourseEmbedAgent extends BaseAgent<{
  ...
  async execute(input: {
    ...
    prompt?: string;
    locale?: string;
  }, ctx?: AgentRunContext): Promise<string> {
    ...
    const currentTranslation = i18n.getAgentPrompt('discourse_embed', 'current_translation', { text: input.target });
    const currentTask = i18n.getAgentPrompt('discourse_embed', 'current_task');
    const optimizationInstruction = i18n.getAgentPrompt('discourse_embed', 'optimization_instruction');
    const preserveOriginalMeaning = i18n.getAgentPrompt('discourse_embed', 'preserve_original_meaning');
    const learnReferenceStyle = i18n.getAgentPrompt('discourse_embed', 'learn_reference_style');
    const ensureTermConsistency = i18n.getAgentPrompt('discourse_embed', 'ensure_term_consistency');
