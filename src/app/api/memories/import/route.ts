...
export async function POST(req: NextRequest) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      payload = Object.fromEntries(Array.from(form.entries()).map(([k, v]) => [k, typeof v === 'string' ? v : (v as any)]));
    } else {
    ...
    if (!fileKey) return NextResponse.json({ success: false, error: 'missing fileKey' }, { status: 400 });
    ...
    if (!memoryId) {
      const mem = await (prisma as any).translationMemory.upsert({
        ...
        update: {},
        create: { id: 'global-memory', name: '全局记忆库', description: '默认导入', tenantId: tenantId || null, userId: userId || null },
      });
      memoryId = mem.id;
    }

    const queue = getQueue('memory-import');
    const job = await queue.add('import', { fileKey, fileType, memoryId, sourceLang, targetLang, tenantId, userId });
    return NextResponse.json({ success: true, data: { jobId: job.id, queue: 'memory-import', memoryId } });
  } catch (e: any) {
