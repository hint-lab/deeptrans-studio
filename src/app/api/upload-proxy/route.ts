...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!contentType.includes('multipart/form-data')) {
      return NextResponse.json({ success: false, error: 'Content-Type 必须为 multipart/form-data' }, { status: 400 });
    }
    ...
    const getUrl = await storageService.getUploadUrl(file.name, file.type || 'application/octet-stream', projectName);
    ...
    if (!putRes.ok) {
      ...
      return NextResponse.json({ success: false, error: `上传失败: ${putRes.status} ${putRes.statusText} ${text}` }, { status: 502 });
    }
    ...
    return NextResponse.json({ success: false, error: e?.message || '上传代理失败' }, { status: 500 });
  }
}
