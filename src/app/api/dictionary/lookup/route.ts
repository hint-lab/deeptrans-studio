...
export async function GET(request: Request) {
  try {
    ...
    console.log('Dictionary API: Query result:', { rowsCount: rows?.length, rows: rows?.slice(0, 3) })
    ...
    const data = (rows || []).map((r: any) => ({
      ...
      source: r?.dictionary ? `${visMap[r.dictionary.visibility as string] || r.dictionary.visibility} · ${r.dictionary.name}` : undefined
    }))

    console.log('Dictionary API: Final response:', { dataCount: data?.length, data: data?.slice(0, 3) })
