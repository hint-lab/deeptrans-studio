...
import { findDocumentsByProjectIdDB, updateDocumentStructuredDB, updateDocumentStatusDB } from '@/db/document' 
...
async function handlePersist(only: any, batchId: string, projectIdFromParams: string) {
  ...
  if (structuredObj) {
    const form = new FormData()
    form.set('file', new Blob([JSON.stringify(structuredObj, null, 2)], { type: 'application/json' }), `${stem}.structured.json`)
    form.set('projectName', projectIdFromParams)
    const res = await uploadFileAction(form as unknown as FormData)
    if ((res as any)?.success) {
  ...
  try { await updateDocumentStructuredDB(only.id, stored) } catch {}
...
export async function POST(req: NextRequest, ctx: any) {
  try {
    ...
    if (action === 'parse') {
      return NextResponse.json({ error: 'moved: use /api/projects/[id]/parse' }, { status: 400 })
    }

    // segment / terms：读取正文（优先从已持久化的 structured.json 构造带 <|n|> 标记的文本，失败再退回原始文本）
    else if (action === 'segment') {
      return NextResponse.json({ error: 'moved: use /api/projects/[id]/segment' }, { status: 400 })
    }

    else if (action === 'terms') {
      return NextResponse.json({ error: 'moved: use /api/projects/[id]/terms' }, { status: 400 })
    }
...
export async function GET(req: NextRequest, ctx: any) {
  try {
    ...
    async function readStatus() {
      ...
      let dict: Array<{ term: string; translation: string; notes?: string; source?: string }> = []
      try {
      ...
      try {
        ...
        if (arr) {
          segments = arr.map((s: any) => ({ type: s?.type, sourceText: String(s?.sourceText || '') }))
        } else {
          // 动态聚合各分块
          ...
          if (total > 0) {
            ...
            for (let i = 0; i < total; i++) {
              ...
              for (const s of parr) parts.push({ type: s?.type, sourceText: String(s?.sourceText || '') })
            }
      ...
      const limitHtml = (() => {
        try {
          ...
          return out
        } catch { return String(fullHtml || '') }
      })()
      // 基于提取的术语做一次精确词典命中查询（限制前 20 个术语），用于前端高亮
      try {
        const session = await auth();
        const userId = session?.user?.id || undefined;
        const uniqueTerms = Array.from(new Set((terms || []).map(t => String(t?.term || '').trim()).filter(Boolean)));
        const out: Array<{ term: string; translation: string; notes?: string; source?: string }> = []
      ...
      try { docxStructured = docxStruct ? JSON.parse(String(docxStruct)) : undefined } catch {}
      return { segProgress, termsProgress, preview: preview || '', previewHtml: fullHtml || '', previewHtmlLimited: limitHtml || '', terms, dict, segments, docxStructured }
    }
    ...
    if (waitMs > 0) {
      ...
      while (Date.now() - start < waitMs) {
        ...
        if ((lastSeg >= 0 && curSeg !== lastSeg) || (lastTerms >= 0 && curTerms !== lastTerms)) {
          const status = await readStatus()
          return NextResponse.json(status)
        }
