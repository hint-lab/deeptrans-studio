...
export async function POST(req: NextRequest, ctx: any) {
  try {
    ...
    const { batchId, mode, autoTranslate, targetLanguage: bodyTL, domain: bodyDomain, documentId } = await req.json().catch(() => ({})) as { batchId: string; mode?: 'append'|'overwrite'|'upsert'; limit?: number; autoTranslate?: boolean; targetLanguage?: string; domain?: string; documentId?: string }
    ...
    let unique: string[] = []
    {
      ...
      if (raw) {
        try {
          ...
          const terms: Array<{ term: string; count?: number; score?: number }> = Array.isArray(obj?.terms) ? obj.terms : []
          unique = Array.from(new Set(terms.map(t => String(t?.term || '').trim()).filter(Boolean)))
        } catch {}
      }
    }
    ...
    if (!unique.length) {
      try {
        ...
        if (only?.url) {
          ...
          if (head) {
            const quick = await extractDocumentTermsAction(head, { maxTerms: 80, chunkSize: 2000, overlap: 200 })
            unique = Array.from(new Set((quick || []).map(t => String(t?.term || '').trim()).filter(Boolean)))
          }
        }
      } catch {}
    }
    ...
    if (!found?.success || !found.data?.id) {
      return NextResponse.json({ error: found?.error || 'dictionary not available' }, { status: 500 })
    }
    const dictionaryId = found.data.id

    const applyMode = (mode || 'upsert') as 'append'|'overwrite'|'upsert'
    const applied = await bulkUpsertEntriesAction({ dictionaryId, projectId, userId: String(userId), terms: unique, mode: applyMode, copyFromOthers: true })
    if (!applied?.success) return NextResponse.json({ error: applied?.error || 'apply failed' }, { status: 500 })
    ...
    if (autoTranslate === true) {
        try {
          ...
          const sourceLanguage = String(q.get('sourceLanguage') || (project as any)?.sourceLanguage || 'auto')
          const targetLanguage = String(q.get('targetLanguage') || bodyTL || (project as any)?.targetLanguage || 'zh')
          const domain = String(q.get('domain') || bodyDomain || (project as any)?.domain || '') || undefined
  
          // 第一次批量翻译
          const batch = await translateTermsBatchAction(srcList, sourceLanguage, targetLanguage, { domain })
          const map = new Map<string, string>(batch.map(x => [norm(x.term), String(x.translation || '').trim()]))
  
          // 缺失项二次批处理兜底
          const missing = srcList.filter((t: any) => !map.get(t))
          if (missing.length > 0) {
            const batch2 = await translateTermsBatchAction(missing, sourceLanguage, targetLanguage, { domain })
            for (const x of batch2) {
              const k = norm(x.term); const v = String(x.translation || '').trim()
          ...
          for (const row of blanks || []) {
            ...
            const tt = map.get(k) || ''
            await updateDictionaryEntryTargetTextDB((row as any).id, tt, tt ? 'apply:mt' : 'apply:new')
            if (tt) updated += 1
          }
        } catch  (e: any) {
    ...
    try {
      ...
      if (only?.id) { await updateDocumentStatusDB(only.id, DocumentStatus.PREPROCESSED as any) }
    } catch {}
