...
export async function POST(req: NextRequest, context: any) {
  try {
    const { batchId, maxTerms, prompt } = await req.json() as { batchId?: string; maxTerms?: number; prompt?: string }
    ...
    for (const t of terms || []) {
      ...
      if (!k) continue
      if (!uniq.has(k)) uniq.set(k, { term: k, score: typeof t?.score === 'number' ? t.score : undefined })
    }
