...
export async function GET(req: NextRequest, context: any) {
  try {
    ...
    if (!item || !item.document) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    ...
    try {
      ...
      const out = new URL(parsed.pathname + parsed.search + parsed.hash, isLocal ? base : parsed.origin);
