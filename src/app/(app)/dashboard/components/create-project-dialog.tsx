...
export function CreateProjectDialog({
    ...
    triggerVariant?: 'default' | 'icon' | 'auto';
}) {
    ...
    const [progressCounts, setProgressCounts] = useState<{ completed?: number; total?: number }>({
        completed: 0,
        total: 3,
    });
    ...
    async function startInitPipelines(projectId: string) {
        try {
            ...
            logger.info(
                `Starting project initialization projectId:${projectId}, batchId:${batchId}`
            );
            ...
            pollRef.current = setInterval(async () => {
                try {
                    const r = await fetch(
                        `/api/projects/${projectId}/init?batchId=${encodeURIComponent(batchId)}`
                    );
                    if (!r.ok) return;
        ...
        <>
            ...
            >
                {(() => {
                    const showDefault =
                        triggerVariant === 'default' ||
                        (triggerVariant === 'auto' && isSidebarOpen);
                    const showIcon =
                        triggerVariant === 'icon' || (triggerVariant === 'auto' && !isSidebarOpen);
                    return (
                        <>
                            {showDefault && (
                                <DialogTrigger asChild>
                                    <Button
                                        variant="default"
                                        className="h-10 w-full justify-center bg-gradient-to-r from-indigo-500 to-purple-600 p-2 text-white shadow-lg transition-all duration-200 hover:from-indigo-600 hover:to-purple-700 hover:shadow-xl"
                                    >
                    ...
                    <div className="grid gap-6 py-4">
                        <div className="grid gap-2">
                            ...
                            <Input
                                ...
                                className="border-gray-200 focus:border-indigo-500 dark:border-gray-700 dark:focus:border-indigo-400"
                            />
                        </div>
                ...
                <UiDialogContent className="sm:max-w-[460px]">
                    ...
                    <div className="space-y-3">
                        <div className="text-sm text-muted-foreground">
                            {t('stage')}: {progressPhase ?? 'INIT'}
                        </div>
                        <Progress value={progressValue} />
                        <div className="text-xs text-muted-foreground">
                            {t('segmentation')}: {segPct}% · {t('terms')}: {termPct}%
                        </div>
                    </div>
                </UiDialogContent>
            </UiDialog>
