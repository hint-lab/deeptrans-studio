...
import { getProjectDictionaryBindingsAction, updateProjectDictionaryBindingsAction, getProjectMemoryBindingsAction, updateProjectMemoryBindingsAction } from "@/actions/project-bindings";

type DictLite = { id: string; name: string };
type MemLite = { id: string; name: string };

export function ProjectDictionariesDialog({ projectId, open, onOpenChange }: { projectId: string; open: boolean; onOpenChange: (v: boolean) => void }) {
        ...
        setLoading(true)
            ; (async () => {
                try {
                    ...
                    if (!cancelled) {
                        if (dictRes?.success) setItems((dictRes.data || []).map((d: any) => ({ id: d.id, name: d.name })))
                ...
                <div className="space-y-3">
                    ...
                    <ScrollArea className="h-64 pr-2">
                        <div className="space-y-2">
                            ...
                            {!items.length && !loading && <div className="text-muted-foreground text-sm">{t('noData')}</div>}
                        </div>
                    </ScrollArea>
                </div>
                <DialogFooter>
...
export function ProjectMemoriesDialog({ projectId, open, onOpenChange }: { projectId: string; open: boolean; onOpenChange: (v: boolean) => void }) {
    const t = useTranslations('Dashboard.ProjectList');
    const [loading, setLoading] = useState(false)
        ...
        setLoading(true)
            ; (async () => {
                try {
                    ...
                    if (!cancelled) {
                        if (memRes?.success) setItems((memRes.data || []).map((m: any) => ({ id: m.id, name: m.name })))
                        if (bindRes?.success) setSelected(bindRes.data || [])
                    }
                } finally { if (!cancelled) setLoading(false) }
            })()
