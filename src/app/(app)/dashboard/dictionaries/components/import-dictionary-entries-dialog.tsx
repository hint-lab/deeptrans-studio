...
export function ImportDictionaryEntriesDialog({ dictionaryId, onCompleted }: ImportDictionaryEntriesDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success(`导入完成: 总计 ${data.data?.total}，新增 ${data.data?.inserted}，更新 ${data.data?.updated}`)
            setOpen(false)
            setFile(null)
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="secondary" size="sm">导入词条</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-lg">
                ...
                <div className="space-y-4 py-2">
                    <div className="space-y-2">
                        ...
                        />
                        <p className="text-xs text-muted-foreground">支持 .xlsx/.xls/.csv/.tbx，建议表头包含 source/target/notes</p>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input placeholder="如 en, zh, ja" value={sourceLang} onChange={(e) => setSourceLang(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label>TBX 目标语言（可选）</Label>
                            <Input placeholder="如 zh, en" value={targetLang} onChange={(e) => setTargetLang(e.target.value)} />
                        </div>
                    </div>
                </div>
                <DialogFooter>
                    <Button variant="outline" onClick={() => setOpen(false)} disabled={submitting}>取消</Button>
                    <Button onClick={handleSubmit} disabled={submitting || !file}>{submitting ? "导入中..." : "开始导入"}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
