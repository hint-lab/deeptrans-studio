...
import { Loader, Play, RefreshCcw, Redo2, Coffee, Loader2, SquareCheckBig, Square } from 'lucide-react'
...
export default function ProjectInitPage() {
  ...
  useEffect(() => { /* 由 useProjectInit.ensure 初始化 batchId；此处不再本地生成 */ }, [projectId])
  ...
  const [dictMatches, setDictMatches] = useState<Array<{ term: string; translation: string; notes?: string; source?: string }>>([])
  ...
  const [termFlow, setTermFlow] = useState<'idle' | 'extracting' | 'applying' | 'translating' | 'done'>('idle')
  const [applyStatsInsert, setApplyStatsInsert] = useState<{ inserted: number; updated: number; skipped: number } | null>(null)
  const [applyStatsTranslate, setApplyStatsTranslate] = useState<{ inserted: number; updated: number; skipped: number } | null>(null)
  ...
  async function startTerms() {
    ...
    setStarting(true)
    try {
      const r = await fetch(`/api/projects/${projectId}/terms`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ batchId, terms: { maxTerms, chunkSize, overlap, prompt: termPrompt } }) })
  ...
  async function loadTermPreview() {
    ...
    try {
      const r = await fetch(`/api/projects/${projectId}/terms/preview`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ batchId, maxTerms, prompt: termPrompt }) })
      const j = await r.json()
      if (Array.isArray(j?.terms)) setTermPreview(j.terms)
    } catch { }
    finally { setTermPreviewLoading(false) }
  }
      ...
      try {
        ...
        try {
          for (let i = 0; i < 4; i++) {
            ...
            if (segs.length) { fullSegs = segs; break }
          }
        } catch { }
  ...
  useEffect(() => {
    ...
    const longPoll = async (prevSeg: number, prevTerms: number) => {
      ...
      // 中止上一轮
      if (statusAbortRef.current) { try { statusAbortRef.current.abort() } catch { } }
      ...
      try {
        ...
        if (typeof j?.previewHtml === 'string' && !previewHtml) setPreviewHtml(j.previewHtml)
        if (!(a >= 100 && b >= 100)) {
          longPoll(a, b)
        } else {
  ...
  useEffect(() => {
    ...
    segDebounceRef.current = setTimeout(() => { loadSegPreview() }, 500)
    return () => { if (segDebounceRef.current) clearTimeout(segDebounceRef.current) }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentStep, segmentDocumentId])
  ...
  return (
    <div className="px-6 py-6">
      <div className="max-w-5xl mx-auto">
        <div className="border rounded-xl bg-white dark:bg-gray-900 mb-5">
            ...
            <div className="flex items-center justify-between">
              ...
              {currentStep === 'done' && (
                <Button size="sm" variant="ghost" onClick={() => {
                  ...
                  restart()
                  updateBatchId(newId)
                }} disabled={starting} className="gap-1">
                  {starting ? <><Loader className="h-4 w-4 animate-spin" />{t('processing')}</> : <><Redo2 className="h-4 w-4" />{t('restart')}</>}
                </Button>
              )}
            </div>
            {/* 横向步骤条 */}
            <Stepper currentStep={currentStep} segPct={segPct} termPct={termPct} onStepClick={(s) => {
              // 仅允许回退：parse <- segment <- terms <- done
              const order: Array<typeof currentStep> = ['parse', 'segment', 'terms', 'done']
          ...
          <div className="px-5 py-6 space-y-6">
            {/* 步骤结果区域 */}
            {currentStep === 'parse' && (
              <ParsePanel previewHtml={previewHtml} />
            )}
            ...
            {currentStep === 'segment' && (
              <SegmentPanel
                ...
                segError={segError}
                onResegment={(o) => { if (segmentDocumentId) void loadSegPreview({ all: o?.all }) }}
            ...
            {currentStep === 'terms' && (
              <TermsPanel
                ...
                onStart={startTerms}
                onApply={async () => { await applyTermsPipeline(); }}
                applying={applyingTerms}
                onViewDictionary={() => router.push(`/dashboard/dictionaries/${projectId}`)}
            ...
            {currentStep === 'done' && (
              <section className="space-y-2" id="step-done">
                <div className="p-4 border rounded-lg bg-emerald-50 border-emerald-300 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-200 dark:border-emerald-800">
            ...
            <div className="pt-2 flex justify-end gap-2">
              {currentStep === 'parse' && (
                <>
                  <Button variant="outline" onClick={() => { updateProgress(0, 0); runParse(); }} disabled={starting}>{t('retryParse')}</Button>
                  <Button onClick={() => { const u = new URL(`/api/projects/${projectId}/init`, window.location.origin); u.searchParams.set('action', 'persist'); u.searchParams.set('batchId', batchId); void fetch(u.toString(), { method: 'POST' }); updateStep('segment'); updateDocumentStatusByIdAction(segmentDocumentId, "PARSING") }} disabled={starting || !previewHtml}>{t('next')}</Button>
                </>
              )}
              {currentStep === 'segment' && (
                <>
                  <Button variant="outline" onClick={() => { if (segmentDocumentId) void loadSegPreview() }} disabled={segLoading || starting}>{t('resegment')}</Button>
                  <Button onClick={handleApplySegmentsClick} disabled={!segItems.length || applying || starting}>
              ...
              {currentStep === 'terms' && (
                <>
                  {termPct < 100 && (
                    ...
                    }} disabled={starting || (termPct > 0 && termPct < 100)}>{t('next')}</Button>
                  )}
                  {false && termPct >= 100 && (
                    <Button variant="outline" onClick={() => { setShowTermApplyModal(true); setTermFlow('applying'); void applyTermsPipeline() }} disabled={applyingTerms}>
                      {applyingTerms ? '写入中…' : '手动写入词库'}
                    </Button>
                  )}
                  {termPct >= 100 && (
                    <Button onClick={() => updateStep('done')}>{t('next')}</Button>
                  )}
                </>
              )}
              {currentStep === 'done' && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-6 w-[360px] text-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-sm font-medium">{applying ? t('applyToFull') : t('prepareApply')}</div>
              <div className="text-xs text-muted-foreground">{t('applyNotice')}</div>
              <div className="text-xs text-muted-foreground">{t('segProgress', { pct: Math.max(0, Math.min(100, segPct)) })}</div>
              <div className="flex gap-2 mt-2">
                {!applying ? (
                  ...
                  <Button variant="outline" disabled>{t('modalBusy')}</Button>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-full text-left text-xs space-y-2">
                <div className="flex items-center justify-between px-2 py-1 rounded border bg-muted/30">
                  ...
                  <div className="text-muted-foreground">{Math.max(0, Math.min(100, termPct))}%</div>
                </div>
                <div className="flex items-center justify-between px-2 py-1 rounded border bg-muted/20">
                  <div className="flex items-center gap-2">
                    {applyStatsInsert ? (
                      <SquareCheckBig className="h-3 w-3 text-emerald-600" />
                    ) : termFlow === 'applying' ? (
                  ...
                  <div className="text-muted-foreground">
                    {applyStatsInsert
                      ...
                      : (termFlow === 'applying' ? t('statusTranslating') : t('statusPending'))}
                  </div>
                </div>
                <div className="flex items-center justify-between px-2 py-1 rounded border bg-muted/10">
                  <div className="flex items-center gap-2">
                    {autoApplyTerms ? (
                      applyStatsTranslate ? (
                        <SquareCheckBig className="h-3 w-3 text-emerald-600" />
                  ...
                  <div className="text-muted-foreground">
                    {autoApplyTerms
                      ? (translateCount !== null
                        ...
                        : (termFlow === 'translating' ? t('statusTranslating') : t('statusPending')))
                      : t('statusOff')}
                  </div>
                </div>
              </div>
              <div className="flex gap-2 mt-2">
                ...
                {termFlow === 'done' && (
                  <Button onClick={() => { setShowTermApplyModal(false); updateStep('done') }}>{t('complete')}</Button>
                )}
              </div>
            </div>
          </div>
