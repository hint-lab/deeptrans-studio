...
const TextTranslationTable = () => {
  ...
  const { sourceLanguage, targetLanguage, setSourceTranslationLanguage, setTargetTranslationLanguage } = useTranslationLanguage();
  ...
  return (
    <div className="flex flex-col h-full w-full">
      {/* 顶部语言选择栏 */}
      <div className="flex items-center justify-between bg-card p-2 rounded-t-lg border-b w-full">
        <div className="flex items-center gap-4 w-full">
          <div className="flex items-center justify-between gap-2 w-1/2">
            <LanguageSelector placeholder="检测源语言" items={languages} value={sourceLanguage} onSelect={setSourceTranslationLanguage} />

            <Button
              variant="ghost"
              onClick={swapLanguages}
              className="p-2  hover:bg-muted transition-colors w-12"
          ...
          <div className="flex items-center gap-2 w-1/2">
            <LanguageSelector placeholder="目标语言" items={languages} value={targetLanguage} onSelect={setTargetTranslationLanguage} />
      ...
      {/* 翻译区域 */}
      <div className="grid grid-cols-1 lg:grid-cols-2 flex-1 border-b">
        {/* 输入区域 */}
...
请点击麦克风图标来翻译语音。"
            ...
            className="flex-1 rounded-none border-none resize-none min-h-[300px] focus-visible:ring-0 focus-visible:ring-offset-0 bg-card p-4"
          />
