...
export default function InstantTranslatePage() {
  ...
  const translationStyles = [
    ...
    { key: 'technical', label: tDashboard('technical'), description: tDashboard('technicalDesc') },
  ...
  }, [sourceText, sourceLanguage, targetLanguage, autoTranslate, translationStyle, debouncedTranslate]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg">
        <div className="flex items-center space-x-4">
          <Select value={sourceLanguage} onValueChange={(value) => setSourceLanguage(value)}>
            <SelectTrigger className="w-40 bg-white dark:bg-gray-700 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600">
        ...
        <div className="flex items-center space-x-4">
          {/* 翻译风格选择 */}
          <Select value={translationStyle} onValueChange={(value) => setTranslationStyle(value)}>
            ...
            <SelectContent className="bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600">
              {translationStyles.map((style) => (
                <SelectItem key={style.key} value={style.key} className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
                  {style.label}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* 原文 */}
        <div className="h-100 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
          ...
          <div className="p-4">
            <Textarea
              ...
              className="h-80 resize-none border-0 focus-visible:ring-0 text-lg shadow-none"
            />
          </div>
        </div>

        {/* 译文 */}
        <div className="h-100 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-800">
            <div className="flex items-center justify-between">
      ...
      <div className="mt-4 flex justify-between text-sm text-gray-500">
        <span>{t("sourceCharCount")}: {sourceText.length}</span>
        <span>{t("targetCharCount")}: {translatedText.length}</span>
      </div>
    </div>
  );
} 