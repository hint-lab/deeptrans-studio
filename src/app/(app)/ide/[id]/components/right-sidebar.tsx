...
const RightSidebar: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-between size-full bg-transparent">
      <div className="flex flex-col items-center">
        <div className="flex items-center justify-center p-1 ">
          <Button
            variant="default"
            onClick={() => { toggleChatMode(); }}
            className={`w-6 h-6 py-2 px-1 relative group overflow-hidden bg-gradient-to-b from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 transition-all duration-200 hover:scale-105 active:scale-95 ${mode === 'chat' ? 'ring-2 ring-blue-300/60' : ''}`}
            asChild
          >
            <div className="flex flex-col items-center justify-center px-1 ">
              <span className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 group-active:opacity-30 transition-opacity"></span>
      ...
      <Dialog open={contactOpen} onOpenChange={setContactOpen}>
        <DialogContent>
          ...
          <div className="space-y-2">
            <Input placeholder={t('contact.titlePlaceholder')} value={contactTitle} onChange={(e) => setContactTitle(e.target.value)} />
            <Textarea rows={6} placeholder={t('contact.messagePlaceholder')} value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} />
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" onClick={() => setContactOpen(false)}>{t('contact.cancel')}</Button>
              <Button
                onClick={() => {
