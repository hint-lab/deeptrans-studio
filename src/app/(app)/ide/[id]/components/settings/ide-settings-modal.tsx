...
export function IdeSettingsModal({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
        ...
        <div className="grid gap-4 py-2">
          <div className="grid gap-2">
            ...
            <div className="max-h-44 overflow-auto rounded-md border p-2 text-sm">
              ...
              {!loading && dicts.length === 0 && <div className="text-muted-foreground">暂无词典</div>}
              {!loading &&
                dicts.map((d) => {
                  const checked = selected.includes(d.id);
                  return (
                    <label key={d.id} className="flex items-center gap-2 py-1 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        className="h-4 w-4"
                        checked={checked}
                        onChange={(e) => {
                          ...
                          setSelected((prev) => (isChecked ? [...prev, d.id] : prev.filter((x) => x !== d.id)));
                        }}
                      />
                      <span>{d.name}</span>
                    </label>
          ...
          <div className="grid gap-2">
            ...
            <div className="max-h-44 overflow-auto rounded-md border p-2 text-sm">
              ...
              {!memoryLoading && memories.length === 0 && <div className="text-muted-foreground">暂无记忆库</div>}
              {!memoryLoading &&
                memories.map((m) => {
                  const checked = selectedMemories.includes(m.id);
                  return (
                    <label key={m.id} className="flex items-center gap-2 py-1 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        className="h-4 w-4"
                        checked={checked}
                        onChange={(e) => {
                          const isChecked = e.target.checked;
                          setSelectedMemories((prev) => (isChecked ? [...prev, m.id] : prev.filter((x) => x !== m.id)));
                        }}
                      />
