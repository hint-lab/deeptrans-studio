...
const ItemStatusDot = ({ status }: { status: string }) => {
  const isHuman = status === 'MT_REVIEW' || status === 'QA_REVIEW' || status === 'POST_EDIT' || status === 'SIGN_OFF';
...
const ItemStatusBadge = ({ status }: { status: string }) => {
  const base = 'px-2 py-[2px] rounded-full whitespace-nowrap border text-[10px] transition-all duration-200';
  const isHuman = status === 'MT_REVIEW' || status === 'QA_REVIEW' || status === 'POST_EDIT' || status === 'SIGN_OFF';
  const isCompleted = status === 'COMPLETED';
...
const ExplorerView = ({ projectId }: { projectId: string }) => {
  ...
  const handleDocumentTabClick = (element: DocumentTab) => {
    if (element.items) {
      ...
      setTimeout(() => { if (listRef.current) listRef.current.scrollTop = prevScroll; }, 0);
    };
  }
  ...
  return (
    <div className="flex flex-col justify-start size-full ">
      <div className="px-2 py-2 bg-muted/40 border-b text-[11px] text-foreground/70 flex items-center justify-between">
      ...
      ) : explorerTabs && explorerTabs.documentTabs && explorerTabs.documentTabs.length > 0 ? (
        <ul
          ref={listRef}
          className="flex flex-col p-2 text-sm text-foreground overflow-y-auto [overflow-anchor:none]"
          data-explorer
          onScroll={(e) => { scrollTopRef.current = (e.currentTarget as HTMLUListElement).scrollTop; }}
        >
            ...
            <React.Fragment key={document.id}>
              <li
                className={`
                  ...
                  ${document.items && expandedFolders[document.id] ? 'bg-accent text-accent-foreground' : ''}
                  font-medium
                `}
                onClick={() => handleDocumentTabClick(document)}
              >
                <FileIcon className="h-4 w-4 shrink-0 flex-none" />
                <span className="whitespace-nowrap text-left overflow-hidden text-ellipsis">
                  {document.name}
                </span>
              </li>
                ...
                <ul className="pl-4">
                  {document.items.map((documentItem) => (
                    <li
                      ...
                      className={`
                        flex items-center gap-2 px-2 py-1.5 cursor-pointer
                        hover:bg-accent hover:text-accent-foreground rounded-sm font-normal select-none
                        ${documentItem.id === activeDocumentItem.id ? 'bg-accent text-accent-foreground' : ''}
                      `}
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap text-left overflow-hidden text-ellipsis text-foreground">
                        {documentItem.name}
                      </span>
                      <span className="ml-auto"><ItemStatusBadge status={documentItem.status as any} /></span>
                    </li>
                  ))}
                </ul>
              )}
