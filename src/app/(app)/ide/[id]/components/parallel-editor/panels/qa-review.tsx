...
import { getDocumentItemIntermediateResultsAction, saveQualityAssureResultsAction, savePreTranslateResultsAction } from "@/actions/intermediate-results";
...
export default function QAPanel({ projectId }: { projectId?: string }) {
  ...
  const qaEmbeddedText = useAgentWorkflowSteps(s => s.qualityAssureSyntaxEmbedded) as string | undefined;
  const setQaSyntaxEmbedded = useAgentWorkflowSteps(s => s.setQASyntaxEmbedded) as (v: string | undefined) => void;
      ...
      try {
        ...
        if (results) {
          ...
          if (results.qualityAssureBiTerm || results.qualityAssureSyntax) {
            ...
            if ((results as any)?.qaDislikedPairs) setQADislikedPairs((results as any).qaDislikedPairs);
            if ((results as any).qualityAssureSyntaxEmbedded) setQaSyntaxEmbedded(String((results as any).qualityAssureSyntaxEmbedded || ''));
          } else {
            setQAOutputs(undefined);
          }
        } else {
  ...
  const dislikedPairs = useAgentWorkflowSteps(s => s.qaDislikedPairs) as Record<string, boolean> | undefined;
  const setQADislikedPairs = useAgentWorkflowSteps(s => s.setQADislikedPairs) as (m: Record<string, boolean> | undefined) => void;
  const toggleIgnore = (term: string) => setIgnoredTerms(prev => ({ ...prev, [term]: !prev[term] }));
  ...
  const applyToTarget = async (text: string) => {
    try {
      ...
      const docId = (activeDocumentItem as any)?.id;
      if (docId) {
        try { await savePreTranslateResultsAction(docId, { targetText: content }); } catch { }
      }
  ...
  const genSyntaxEmbedded = async () => {
    try {
      if (!sourceText) { toast.error(t('noSource')); return; }
      setLoadingEmbedded(true);
      const syn = asJson(qaSyntax);
      const allIssues: Array<{ type?: string; span?: string; advice?: string }> = Array.isArray(syn?.issues) ? syn.issues : [];
      const selectedMap: Record<string, boolean> = (syn?.selectedMap || {}) as Record<string, boolean>;
      const filteredIssues = allIssues.filter((it, idx) => {
        const key = (() => {
          const t = String(it?.type || '').trim().toLowerCase();
          const s = String(it?.span || '').trim().toLowerCase();
          const a = String(it?.advice || '').trim().toLowerCase();
          return `${t}|${s}|${a}` || `idx:${idx}`;
        })();
        if (Object.keys(selectedMap || {}).length === 0) return true;
        return selectedMap?.[key] === true;
      });
      const text = await embedSyntaxAdviceAction(sourceText || '', targetText || '', filteredIssues);
      setQaSyntaxEmbedded(text || '');
      const docId = (activeDocumentItem as any)?.id;
      if (docId) { try { await saveQualityAssureResultsAction(docId, { syntaxEmbedded: text }); } catch { } }
    } catch (e: any) {
  ...
  function normalizePairs(input: any): Array<{ source: string; target: string; score?: number }> {
    ...
    const push = (src: any, tgt: any, sc?: any) => {
      ...
      if (s || t) out.push({ source: s, target: t, score: typeof sc === 'number' ? sc : undefined });
    };
    const val = asJson(input) ?? input;
    const candidates: any = Array.isArray(val)
      ? val
      : (Array.isArray((val as any)?.pairs) ? (val as any).pairs
        : Array.isArray((val as any)?.data) ? (val as any).data
          : Array.isArray((val as any)?.items) ? (val as any).items
            : Array.isArray((val as any)?.syntaxPairs) ? (val as any).syntaxPairs
              : undefined);
    if (Array.isArray(candidates)) {
      for (const it of candidates) {
        if (it && typeof it === 'object') {
          push(
            (it as any).source ?? (it as any).src ?? (it as any).term ?? (it as any).sourceMarker,
            (it as any).target ?? (it as any).tgt ?? (it as any).translation ?? (it as any).targetMarker,
  ...
  function buildDislikedPrompt(pairs: Array<{ source: string; target: string }>, dislikedMap: Record<string, boolean>): string | undefined {
    ...
    const lines = dislikedList.map((x) => t('dislikedPairFormat', { source: x.source, target: x.target })).join('\n');
  ...
  return (
    <div className="flex flex-col size-full border rounded p-2 bg-purple-50 border-purple-200 dark:bg-purple-950/30 dark:border-purple-800">
      <div className="flex items-center justify-between">
        <div className="text-xs text-foreground/70 font-medium">{t('headerSyntaxQA')}</div>
      </div>

      {/* 三栏布局（双语对齐 / 句法建议 / 术语嵌入译文） */}
      <ResizablePanelGroup direction="horizontal" className="mt-2 h-full items-stretch" onLayout={(sizes: number[]) => { try { document.cookie = `react-resizable-panels:qa-review-layout=${JSON.stringify(sizes)}` } catch { } }}>
        {/* 双语句法评估 */}
        <ResizablePanel defaultSize={33} minSize={20} className={cn("rounded p-2 bg-white dark:bg-slate-900", panelCls)}>
          <div className="text-[11px] text-foreground mb-1 font-semibold">{t('headerSyntaxQA')}</div>
          {(() => {
            const bi = asJson(qaBiTerm);
            const pairs: Array<{ source: string; target: string; score?: number }> = normalizePairs(qaBiTerm).map(p => ({ source: p.source, target: p.target, score: p.score }));
            const coverage: number | undefined = typeof bi?.coverage === 'number' ? bi.coverage : undefined;
            if (!bi) return <div className="text-foreground/60 text-xs">{t('notRun')}</div>;
            return (
              <div className="overflow-auto text-xs space-y-2">
                {(typeof coverage === 'number') && (
                  <div className="flex items-center gap-2">
                    <span className="text-foreground/60">{t('coverage')}</span>
                    <div className="h-1.5 w-40 bg-slate-200 rounded">
                      <div className="h-1.5 bg-purple-500 rounded" style={{ width: `${Math.max(0, Math.min(100, Math.round(coverage * 100)))}%` }} />
                    </div>
                    <span className="text-foreground/70">{Math.round(coverage * 100)}%</span>
                  </div>
                )}
                {pairs.length ? (
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-foreground/60">
                        <th className="text-left font-normal px-2 py-1 w-1/3">{t('colSourceTerm')}</th>
                        <th className="text-left font-normal px-2 py-1 w-1/3">{t('colAlignedTranslation')}</th>
                        <th className="text-left font-normal px-2 py-1">{t('colScore')}</th>
                        <th className="text-left font-normal px-2 py-1 w-[60px] text-center">{t('colDislike')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {pairs.map((p, i) => {
                        ...
                        const rowCls = cn("border-t", noted ? "bg-purple-50/70 dark:bg-purple-900/20" : "");
                        const disliked = (dislikedPairs || {})[key] === true;
                        const toggleDislike = async () => {
                          ...
                          const docId = (activeDocumentItem as any)?.id;
                          if (docId) {
                            try { await saveQualityAssureResultsAction(docId, { dislikedPairs: next }); } catch { }
                          }
                        };
                        const onToggle = () => toggleNote(p.source);
                        return (
                          <tr key={key} className={rowCls} onClick={onToggle} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onToggle(); } }}>
                            <td className="px-2 py-1 break-words align-top">{p.source}</td>
                            <td className="px-2 py-1 break-words align-top">{p.target}</td>
                            <td className="px-2 py-1 align-top">
                              {typeof p.score === 'number' ? (
                                <div className="flex items-center gap-2">
                                  <div className="h-1 w-20 bg-slate-200 rounded">
                                    <div className="h-1 bg-emerald-500 rounded" style={{ width: `${Math.max(0, Math.min(100, Math.round(p.score * 100)))}%` }} />
                                  </div>
                                  <span className="text-foreground/70">{Math.round(p.score * 100)}%</span>
                                </div>
                              ) : (
                                <span className="text-foreground/60">—</span>
                              )}
                            </td>
                            <td className="px-2 py-1 align-top text-center" onClick={(e) => { e.stopPropagation(); toggleDislike(); }}>
                              <button className={cn("inline-flex items-center justify-center rounded p-1 border", disliked ? "border-red-300 bg-red-50 text-red-600" : "border-slate-200 text-foreground/60 hover:bg-muted")}
                                title={disliked ? t('markedAsPoor') : t('markAsPoor')}
                              >
        ...
        {/* 句法特征评估 */}
        <ResizablePanel defaultSize={33} minSize={20} className={cn("rounded p-2 bg-white dark:bg-slate-900", panelCls)}>
          <div className="text-[11px] text-foreground mb-1 font-semibold">{t('syntaxFeatureAssessment')}</div>
          {(() => {
            const syn = asJson(qaSyntax);
            const issues: Array<{ type: string; span?: string; advice?: string }> = syn?.issues || [];
            const selectedMap: Record<string, boolean> = (syn?.selectedMap || {}) as Record<string, boolean>;
            const makeKey = (it: { type?: string; span?: string; advice?: string }, idx: number) => {
              const t = String(it?.type || '').trim().toLowerCase();
              const s = String(it?.span || '').trim().toLowerCase();
              const a = String(it?.advice || '').trim().toLowerCase();
            ...
            const toggleSelect = async (key: string) => {
              try {
                ...
                const nextMap = { ...(selectedMap || {}), [key]: nextSelected } as Record<string, boolean>;
                const nextSyn = { ...(syn || {}), selectedMap: nextMap } as any;
                setQAOutputs({ syntax: nextSyn });
                const docId = (activeDocumentItem as any)?.id;
                if (docId) {
                  try { await saveQualityAssureResultsAction(docId, { syntax: nextSyn }); } catch { }
                }
              } catch { }
            };
            if (!syn) return <div className="text-foreground/60 text-xs">{t('notDetected')}</div>;
            return (
              <div className="overflow-auto text-xs space-y-1">
                {issues.length ? issues.map((it, i) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={cn(
                        "flex items-start gap-2 border rounded px-2 py-1 cursor-pointer hover:border-emerald-300/60",
                        checked ? "border-emerald-300 bg-emerald-50/40 dark:bg-emerald-900/10" : ""
                      )}
                      role="button"
                      tabIndex={0}
                    ...
                    >
                      <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 text-[10px] whitespace-nowrap">{it.type || t('issueTag')}</span>
                      <div className="flex-1">
                        {it.span && <div className="text-foreground/80 break-words">{it.span}</div>}
                        {it.advice && <div className="text-foreground/60">{t('advicePrefix')} {it.advice}</div>}
                      </div>
                      {checked && (
                ...
                }) : <div className="text-foreground/60">No obvious issues found</div>}
              </div>
            );
          })()}
        </ResizablePanel>
        <ResizableHandle withHandle className="bg-transparent" />
        {/* 句法嵌入翻译 */}
        <ResizablePanel defaultSize={34} minSize={20} className={cn("rounded p-2 bg-white dark:bg-slate-900 flex-1 ", panelCls)}>
          <div className="flex items-center justify-between">
            <div className="text-[11px] text-foreground mb-1 font-semibold">{t('syntaxEmbedding')}</div>
            <div className="flex items-center gap-2">
              <label className="flex items-center gap-1 text-[11px] text-foreground/70">
                <input type="checkbox" disabled={!baseline && !qaEmbeddedText} checked={!!showDiff} onChange={() => setShowDiff(v => !v)} />
                {t('showDiff')}
              </label>
              <Button
                size="sm"
          ...
          {(() => {
            ...
            const d = showDiff && baseline !== null && qaEmbeddedText !== null ? wordDiff(a, b) : null;
            return (
                ...
                {hasAny && (
                  <>
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onClick={() => { if (a) { applyToTarget(String(a)); setAppliedKind('baseline'); } }}
                      onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && a) { e.preventDefault(); applyToTarget(String(a)); setAppliedKind('baseline'); } }}
                    >
                    ...
                    {qaEmbeddedText !== null && (
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        onClick={() => { if (b) { applyToTarget(String(b)); setAppliedKind('embedded'); } }}
                        onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && b) { e.preventDefault(); applyToTarget(String(b)); setAppliedKind('embedded'); } }}
                      >
                        <div className="flex items-center justify-between text-[11px] mb-1 w-full">
