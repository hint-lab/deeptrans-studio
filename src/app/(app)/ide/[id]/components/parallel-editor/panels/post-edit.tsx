...
export default function PostEditPanel() {
  ...
  const posteditDiscourse = useAgentWorkflowSteps(s => s.posteditDiscourse) as EvaluationResult | undefined;
    ...
    const loadResults = async () => {
      ...
      try {
        ...
        if (results) {
          ...
          if (results.postEditQuery || results.postEditEvaluation || results.postEditRewrite) {
            setPosteditOutputs({
  ...
  const handleRegenerateEmbedding = async () => {
    ...
    try {
      ...
      const sourceFromStore = (typeof (globalThis as any).__deeptransSource === 'string') ? (globalThis as any).__deeptransSource : '';
      const sourceTextToUse = source || sourceFromStore;
      if (!sourceTextToUse) {
        // 如果拿不到源文，退化为用目标文做自回译式改写
        console.warn('未获取到源文，将仅基于目标文与参考进行改写');
      }
      const rewritten = await embedDiscourseAction(sourceTextToUse, String(targetText), refs as any, {
        tenantId: (activeDocumentItem as any)?.projectId || (activeDocumentItem as any)?.tenantId,
      });
      setPosteditOutputs({
        memos: posteditMemos,
        discourse: posteditDiscourse,
  ...
  return (
    <div className="flex flex-col w-full h-full border rounded p-2 bg-orange-50 border-orange-200 dark:bg-orange-950/30 dark:border-orange-800">
      <div className="text-xs text-foreground/70 font-medium mb-2">{t('title')}</div>
      ...
      >
        ...
        <ResizablePanel
          defaultSize={33}
          minSize={20}
          className={cn("rounded p-2 bg-white dark:bg-slate-900", panelCls)}
        >
          <div className="flex items-center justify-between">
            <div className="text-[11px] text-foreground mb-1 font-semibold">{t('discourseQuery')}</div>
            <div className="text-xs text-foreground/60 mb-1">
              {t('totalResults')} {queryResults.length} 条
              {selectedReferences.length > 0 && ` · ${t('selectedResults')} ${selectedReferences.length} 条`}
            </div>
          </div>
            ...
            {queryResults.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <div className="text-sm text-muted-foreground">
                  <p className="font-medium mb-1">{t('noTranslation')}</p>
                  <p>{t('performQuery')}</p>
                </div>
              </div>
            ) : (
              queryResults.map((hit, index) => (
                <div key={hit.id} className={cn(
                  "border rounded p-2 hover:shadow-sm transition-shadow cursor-pointer",
                  isSelected(hit)
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleSelectionChange(hit, checked as boolean)}
                        className="mr-1"
                      />
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-medium ${getScoreColor(hit.score)}`}>
                  ...
                  <Progress value={Math.round(hit.score * 100)} className="h-1 mb-2" />
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          size="sm"
                          className="h-4 px-1"
                          onClick={() => copyToClipboard(hit.source, `source-${hit.id}`)}
                        >
                      ...
                      <div className="text-xs leading-relaxed p-1 bg-muted/30 rounded">
                        {hit.source}
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          size="sm"
                          className="h-4 px-1"
                          onClick={() => copyToClipboard(hit.target, `target-${hit.id}`)}
                        >
                      ...
                      <div className="text-xs leading-relaxed p-1 bg-orange-50 dark:bg-orange-950/20 rounded font-medium">
                        {hit.target}
                      </div>
                    </div>
                  </div>
        ...
        <ResizablePanel
          ...
          className={cn("rounded p-2 bg-white dark:bg-slate-900", panelCls)}
        >
          <div className="text-[11px] text-foreground mb-1 font-semibold">{t('qualityAssessment')}</div>
          ...
          <div className="h-full overflow-auto text-xs space-y-3">
            {!evaluation ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <div className="text-sm text-muted-foreground">
                  <p className="font-medium mb-1">{t('noAssessment')}</p>
              ...
              <div className="space-y-4">
                ...
                {evaluation.styleMatchScore !== undefined && (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium">{t('styleConsistency')}</span>
                      <div className="flex items-center gap-2">
                        {getScoreIcon(evaluation.styleMatchScore)}
                        <span className={`text-xs font-medium ${getScoreColor(evaluation.styleMatchScore)}`}>
                    ...
                    <Progress value={Math.round(evaluation.styleMatchScore * 100)} className="h-1.5" />
                ...
                {evaluation.consistencyScore !== undefined && (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium">{t('terminologyConsistency')}</span>
                      <div className="flex items-center gap-2">
                        {getScoreIcon(evaluation.consistencyScore)}
                        <span className={`text-xs font-medium ${getScoreColor(evaluation.consistencyScore)}`}>
                    ...
                    <Progress value={Math.round(evaluation.consistencyScore * 100)} className="h-1.5" />
                ...
                {evaluation.wordChoiceScore !== undefined && (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-medium">{t('wordChoice')}</span>
                      <div className="flex items-center gap-2">
                        {getScoreIcon(evaluation.wordChoiceScore)}
                        <span className={`text-xs font-medium ${getScoreColor(evaluation.wordChoiceScore)}`}>
                    ...
                    <Progress value={Math.round(evaluation.wordChoiceScore * 100)} className="h-1.5" />
                ...
                {evaluation.recommendations && evaluation.recommendations.length > 0 && (
                  <div className="space-y-2">
                    <div className="text-xs font-medium">{t('suggestions')}</div>
                    <div className="space-y-1">
                      {evaluation.recommendations.map((rec, index) => (
                        <div key={index} className="border border-blue-200 bg-blue-50/50 dark:bg-blue-950/20 rounded p-2">
                ...
                {evaluation.overallScore !== undefined && (
                  <div className="border-t pt-3">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium">{t('overallScore')}</span>
                        <div className="flex items-center gap-2">
                          {getScoreIcon(evaluation.overallScore)}
                          <span className={`text-xs font-medium ${getScoreColor(evaluation.overallScore)}`}>
                      ...
                      <Progress value={Math.round(evaluation.overallScore * 100)} className="h-1.5" />
                    </div>
                  </div>
                )}
              </div>
        ...
        <ResizablePanel
          defaultSize={34}
          minSize={20}
          className={cn("rounded p-2 bg-white dark:bg-slate-900 flex-1", panelCls)}
        >
          <div className="flex items-center justify-between">
            <div className="text-[11px] text-foreground mb-1 font-semibold">{t('discourseEmbedding')}</div>
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                className="h-6 px-2 text-[11px]"
                onClick={handleRegenerateEmbedding}
                disabled={regenerating || (queryResults.length === 0 && selectedReferences.length === 0)}
                title={selectedReferences.length > 0 ? t('reEmbedBasedOnSelected') : (queryResults.length > 0 ? t('reEmbedBasedOnAll') : t('noReference'))}
              >
                {regenerating ? (<><Loader2 className="h-3 w-3 mr-1 animate-spin" />{t('reEmbedding')}</>) : t('reEmbed')}
              </Button>
          ...
          {(() => {
            ...
            const d = showDiff && targetText !== null && enhancedTarget !== null ? wordDiff(a, b) : null;
            return (
                  ...
                  <>
                    <div
                      className={cn(
                        ...
                        appliedKind === 'original' ? "border-emerald-400 bg-emerald-50/40 dark:bg-emerald-900/10" : "border-slate-200 dark:border-slate-800"
                      )}
                      role="button"
                      tabIndex={0}
                      onClick={() => { if (a) { handleApplyToEditor(String(a), 'original'); } }}
                      onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && a) { e.preventDefault(); handleApplyToEditor(String(a), 'original'); } }}
                    >
                      <div className="flex items-center justify-between text-[11px] mb-1 w-full">
                        <div className={cn("flex justify-between items-center gap-2 w-full", appliedKind === 'original' ? 'text-emerald-700 dark:text-emerald-300' : 'text-foreground/60')}>
                          <span>{t('originalTranslation')}</span>
                          {appliedKind === 'original' && <Badge variant="outline" className="h-4 px-1 py-0 text-[10px] border-emerald-300 text-emerald-700">{t('applied')}</Badge>}
                        </div>
                      </div>
                    ...
                    {enhancedTarget && (
                      <div
                        className={cn(
                          "border rounded-md p-2 bg-muted/30 cursor-pointer hover:border-emerald-300/60 transition-colors",
                          appliedKind === 'enhanced' ? "border-emerald-400 bg-emerald-50/40 dark:bg-emerald-900/10" : "border-slate-200 dark:border-slate-800"
                        )}
                        role="button"
                        tabIndex={0}
                        onClick={() => { if (b) { handleApplyToEditor(String(b), 'enhanced'); } }}
                        onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && b) { e.preventDefault(); handleApplyToEditor(String(b), 'enhanced'); } }}
                      >
                        <div className="flex items-center justify-between text-[11px] mb-1 w-full">
                          <div className={cn("flex justify-between items-center gap-2 w-full", appliedKind === 'enhanced' ? 'text-emerald-700 dark:text-emerald-300' : 'text-foreground/60')}>
                            <span>{t('enhancedTranslation')}</span>
                            {appliedKind === 'enhanced' && <Badge variant="outline" className="h-4 px-1 py-0 text-[10px] border-emerald-300 text-emerald-700">{t('applied')}</Badge>}
                          </div>
                        </div>
                        <div className="whitespace-pre-wrap break-words">
                          {d ? (
