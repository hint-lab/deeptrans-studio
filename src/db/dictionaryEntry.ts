 ...
 */
export const findDictionaryEntriesByDictionaryIdDB = async (
    dictionaryId: string,
    limit?: number
): Promise<DictionaryEntry[] | null> => {
    return dbTry(() =>
...
/** 通用查询：where + 分页 */
export const findDictionaryEntriesDB = async (
    where: any,
    skip?: number,
    take?: number
): Promise<DictionaryEntry[] | null> => {
...
// 删除
/** 按词典 ID 批量删除条目 */
export const deleteDictionaryEntriesByDictionaryIdDB = async (
    dictionaryId: string
): Promise<{ count: number } | null> => {
...
export const findExistingDictionaryEntriesMapDB = async (
    dictionaryId: string,
    sources: string[]
): Promise<Map<string, string>> => {
    const rows = await dbTry(() =>
        prisma.dictionaryEntry.findMany({
            where: { dictionaryId, sourceText: { in: sources } },
            select: { id: true, sourceText: true },
        })
    );
    ...
    return new Map<string, string>(
        list.map((r: { id: string; sourceText: string }) => [r.sourceText, r.id])
    );
};
...
// 更新
export const updateDictionaryEntryByIdDB = async (
    id: string,
    data: {
        sourceText?: string;
        targetText?: string;
...
export const findCandidateTranslationsForSourcesDB = async (
    ...
    const projectDictIds = projectId
        ? await prisma.projectDictionary
              .findMany({
                  where: { projectId },
    ...
    const dictWhere: any = {
        OR: [
            { visibility: 'PUBLIC' },
            // 项目级：只保留“本项目”绑定的字典
            ...(projectDictIds.length
                ? [{ visibility: 'PROJECT', id: { in: projectDictIds } }]
                : []),
            // 私有：当前用户拥有
...
export const findByScopeDB = async (
    term: string,
    orScopes: any[],
    limit: number
): Promise<Array<{
...
export const findExactByScopeDB = async (
    term: string,
    orScopes: any[],
    limit: number
): Promise<
