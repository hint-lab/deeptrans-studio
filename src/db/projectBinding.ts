...
export const listProjectDictionaryBindingsDB = async (projectId: string) => {
    return dbTry(() =>
        prisma.projectDictionary.findMany({
            where: { projectId },
            select: {
                dictionaryId: true,
                dictionary: { select: { id: true, name: true, visibility: true } },
            },
...
export const updateProjectDictionaryBindingsDB = async (
    ...
    return dbTry(async () =>
        prisma.$transaction(async (tx: any) => {
            // 删除未在列表中的绑定
            await tx.projectDictionary.deleteMany({
                where: { projectId, ...(unique.length ? { dictionaryId: { notIn: unique } } : {}) },
            });
            // 确保存在列表中的绑定
            for (const did of unique) {
                const exist = await tx.projectDictionary.findUnique({
                    where: { projectId_dictionaryId: { projectId, dictionaryId: did } } as any,
                });
                if (!exist) {
    ...
    return dbTry(async () =>
        prisma.$transaction(async (tx: any) => {
            await tx.projectMemory.deleteMany({
                where: { projectId, ...(unique.length ? { memoryId: { notIn: unique } } : {}) },
            });
            for (const mid of unique) {
                const exist = await tx.projectMemory.findUnique({
                    where: { projectId_memoryId: { projectId, memoryId: mid } },
                });
