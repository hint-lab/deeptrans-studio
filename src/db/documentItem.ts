...
// 批量创建
export async function createDocumentItemsBulkDB(items: DocumentItemCreateInput[]): Promise<{ count: number }> {
...
// 查找：按 documentId 分页
export async function findDocumentItemsByDocumentIdDB(documentId: string, opts?: { skip?: number; take?: number; order?: 'asc' | 'desc' }): Promise<DocumentItem[]> {
...
// 更新：按 id 部分字段
export async function updateDocumentItemByIdDB(id: string, data: DocumentItemUpdateInput): Promise<DocumentItem> {
  const payload: any = {};
  const keys = [
    'sourceText', 'targetText', 'status', 'type', 'metadata',
    'preTranslateTerms', 'preTranslateDict', 'preTranslateEmbedded',
    'qualityAssureBiTerm', 'qualityAssureSyntax', 'qualityAssureSyntaxEmbedded',
    'postEditDiscourse', 'postEditEmbedded', 'needsReview', 'locked',
  ] as const;
  for (const k of keys) {
...
// 删除：按 documentId 批量
export async function deleteDocumentItemsByDocumentIdDB(documentId: string): Promise<{ count: number }> {
  return prisma.documentItem.deleteMany({ where: { documentId } });
}

// 通过一组 id 批量查询（轻量字段）
export async function findDocumentItemsLiteByIdsDB(ids: string[]): Promise<Array<{ id: string; sourceText: string | null; targetText?: string | null; documentId?: string }>> {
  if (!Array.isArray(ids) || !ids.length) return [] as any[];
  return prisma.documentItem.findMany({ where: { id: { in: ids } }, select: { id: true, sourceText: true, targetText: true, documentId: true } }) as any;
}
...
// 按文档导出源文/译文文本
export async function findDocumentItemTextPairsByDocumentIdDB(documentId: string): Promise<Array<{ sourceText: string | null; targetText: string | null }>> {
  if (!documentId) return [];
  return prisma.documentItem.findMany({ where: { documentId }, orderBy: { order: 'asc' as any }, select: { sourceText: true, targetText: true } });
}
...
export async function fetchDocumentItemNeedsMtReviewByIdDB(id: string): Promise<boolean> {
  const row = await prisma.documentItem.findUnique({ where: { id }, select: { needsMtReview: true } });
