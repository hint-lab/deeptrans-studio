...
import { ensure, reset, setBatchId, setProgress, setStep, type InitStep } from '@/store/features/projectInitSlice'
...
export function useProjectInit(projectId: string | undefined) {
  ...
  const updateBatchId = useCallback((next: string) => {
    if (!pid) return
  ...
  const updateStep = useCallback((step: InitStep) => {
    if (!pid) return
    dispatch(setStep({ projectId: pid, step }))
  }, [dispatch, pid])

  const updateProgress = useCallback((segPct?: number, termPct?: number) => {
    if (!pid) return
